    ...
    if serPortn == 1
        ...
        try
            ...
            handles.timer = timer(...
                ...
                'TimerFcn', {@check_status,hObject}); % Specify callback function.
...
status.guard2Err = isempty(strfind(statArray{21}, ' circuit 2 = 1'));
status.errState = any([status.preamp1Err, status.guard1Err, ...
status.guard2Err, status.interlockOn, status.keyOff]);

if status.errState
    axes(handles.status_indicator);
    rectangle('Curvature',[1 1], 'FaceColor', 'yellow');
    axis off equal
    set(handles.err_text, 'Undefined Error')
    set(handles.open_push, 'Enable', 'Off');
end
if status.keyOff
    axes(handles.status_indicator);
    rectangle('Curvature',[1 1], 'FaceColor', 'yellow');
    axis off equal
    set(handles.err_text, 'Key locked')
    set(handles.open_push, 'Enable', 'Off');
end
if status.interlockOn
    ...
    rectangle('Curvature',[1 1], 'FaceColor', 'yellow');
    axis off equal
    set(handles.err_text, 'Interlock active')
    set(handles.open_push, 'Enable', 'Off');
end
if status.preamp1Err
    ...
    rectangle('Curvature',[1 1], 'FaceColor', 'red');
    axis off equal
    set(handles.err_text, 'Preamp error')
    set(handles.open_push, 'Enable', 'Off');
end
